...
export function AchievementsList({ friendId }: { friendId?: string }) {
  ...
  const query = useQuery({
    queryKey: friendId
      ? queryKeys.friendAchievements(friendId)
      : queryKeys.achievements,
    queryFn: ({ signal }) =>
      friendId
        ? getFriendAchievements(friendId, signal)
  ...
  if (query.isLoading || query.isError) {
    return (
      <ScreenState
        ...
        title={
          query.isError
            ? "Не вдалося завантажити нагороди"
            : "Завантажуємо нагороди"
        }
  ...
  return (
    <View style={styles.list}>
      {query.data?.map((item, index) => {
        ...
        return (
          <AnimatedSection key={item.listKey} index={index}>
            <View
              style={styles.card}
              accessible
              accessibilityLabel={`${meta.label}: ${item.count} з ${target}`}
            >
              ...
              <View style={styles.body}>
                ...
                <View
                  ...
                  accessibilityValue={{
                    min: 0,
                    max: target,
                    now: Math.min(item.count, target),
                  }}
                  style={styles.track}
                >
                  <View
                ...
                <Text style={styles.hint}>
                  {item.nextTier
                    ? `Ще ${Math.max(item.nextTier - item.count, 0)} до наступного рівня`
                    : "Усі рівні відкрито"}
                </Text>
              </View>
            </View>
