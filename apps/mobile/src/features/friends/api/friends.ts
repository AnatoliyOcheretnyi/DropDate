...
export const getFriends = (signal?: AbortSignal) =>
  apiRequest<FriendsResponse>("/friends", { auth: true, signal });

export async function searchFriends(query: string, signal?: AbortSignal) {
  const response = await apiRequest<{ results?: FriendSearchResult[] }>(
...
export const sendFriendRequest = (query: string) =>
...
export const respondFriendRequest = (friendshipId: string, accept: boolean) =>
  apiRequest<void>("/friends/requests/respond", {
    method: "POST",
    auth: true,
    body: { friendshipId, accept },
  });
export const removeFriendship = (friendshipId: string) =>
  apiRequest<void>(
    `/friends?friendshipId=${encodeURIComponent(friendshipId)}`,
    { method: "DELETE", auth: true },
  );

export async function getFriendSaved(friendId: string, signal?: AbortSignal) {
  const response = await apiRequest<{ items?: SavedItem[] }>(
    `/friends/saved?friendId=${encodeURIComponent(friendId)}`,
    { auth: true, signal },
  );
