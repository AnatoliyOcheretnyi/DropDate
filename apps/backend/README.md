...
Minimal Go HTTP API that powers the `/next-release` endpoint and feeds the DropDate frontend/mobile clients. It currently relies solely on TMDB for data and keeps an in-memory cache to avoid hammering the public API.
